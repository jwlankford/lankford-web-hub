            ...
            <div v-if="isAdmin" ref="userMenuContainer" class="relative">
              ...
              >
                ...
                <div class="py-1.5 text-xs font-medium">
                  ...
                  <!-- Reset Cache / Sync with Windows OS -->
                  <button 
                    @click="resetToSystemTheme(); isUserMenuOpen = false;"
                    class="w-full px-4 py-2 text-left hover:bg-slate-100 dark:hover:bg-slate-800/80 flex items-center justify-between text-slate-700 dark:text-slate-200 transition-colors"
                    title="Clear cached preference and sync theme with Windows OS mode"
                  >
                    <div class="flex items-center space-x-2.5">
                      <svg class="w-4 h-4 text-cyan-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                      </svg>
                      <span>Sync with Windows OS</span>
                    </div>
                  </button>
