...
<template>
  <div id="signup-form" class="relative bg-slate-900 border border-slate-800 rounded-3xl p-8 sm:p-12 shadow-2xl overflow-hidden max-w-4xl mx-auto">
    ...
    <div class="max-w-2xl mx-auto text-center space-y-4 relative z-10">
      <span class="text-xs font-mono text-cyan-400 uppercase tracking-widest bg-cyan-950/80 px-3 py-1 rounded-full border border-cyan-500/30">
        Reader Community
      </span>
      ...
      <p class="text-slate-300 text-sm sm:text-base leading-relaxed">
        Be the first to receive free preview chapters, technical architectural breakdowns, and release notifications for <strong class="text-blue-400">Today's Software Developer</strong>.
      </p>

      <form @submit.prevent="handleSubmit" class="pt-4 space-y-4 text-left">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-mono text-slate-300 font-semibold mb-1">First Name (Optional)</label>
            <input 
              v-model="firstName"
              type="text"
              placeholder="e.g. Alex"
              class="w-full bg-slate-950 border border-slate-700/80 rounded-xl px-4 py-3 text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors text-sm"
            />
          </div>

          <div>
            <label class="block text-xs font-mono text-slate-300 font-semibold mb-1">Email Address *</label>
            <input 
              v-model="email"
              type="email"
              required
      ...
      <p class="text-[11px] text-slate-500 font-mono pt-2">
        Zero spam guarantee. Unsubscribe at any time with one click. Bound to the <span class="text-blue-400">jeremylankford.com</span> domain context.
      </p>
    </div>
  </div>
</template>
