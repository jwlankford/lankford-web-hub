...
<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-emerald-500 selection:text-white pb-16 transition-colors duration-300">
    <!-- Navigation & Header -->
    <Header
      :activeTab="activeTab"
      ...
      @openAdminModal="isAdminModalOpen = true"
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
      ...
      <div v-if="activeTab === 'papers'" class="space-y-6">
          ...
          <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="relative w-full sm:max-w-md">
              ...
              <input 
                ...
                class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700/80 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-colors shadow-inner"
              />
              <button 
                v-if="searchQuery"
                @click="searchQuery = ''"
            ...
            <div class="flex items-center space-x-3 text-xs font-mono text-slate-500 dark:text-slate-400">
              <span>Showing <strong class="text-emerald-600 dark:text-emerald-400">{{ filteredPapers.length }}</strong> of {{ papers.length }} Studies</span>
              <button 
          ...
          <div v-if="allTags.length" class="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-200 dark:border-slate-800/80">
            ...
            <button
              ...
              :class="[
                ...
                !selectedTagSlug ? 'bg-emerald-600 text-white font-semibold' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-400 hover:bg-slate-300 dark:hover:bg-slate-700'
              ]"
            >
              All Topics
            </button>
        ...
        <!-- Papers Grid -->
        <div v-if="isLoading" class="py-16 text-center text-slate-500 dark:text-slate-400 font-mono">
        ...
        <div v-else-if="filteredPapers.length === 0" class="py-16 text-center bg-white dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800">
          ...
          <button 
            @click="selectedTagSlug = null; searchQuery = '';"
            class="text-xs font-mono px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-emerald-600 dark:text-emerald-400 rounded-lg border border-slate-300 dark:border-slate-700"
          >
            Reset Filters
          </button>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-2xl shadow-xl transition-colors">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-slate-700 dark:text-slate-300">
              <thead class="bg-slate-100 dark:bg-slate-950 font-mono text-emerald-700 dark:text-emerald-400 uppercase border-b border-slate-200 dark:border-slate-800">
                <tr>
              ...
              <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                <tr v-for="paper in papers" :key="paper.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td class="p-3 font-mono text-emerald-700 dark:text-emerald-400/90 whitespace-nowrap">{{ paper.authors }}</td>
