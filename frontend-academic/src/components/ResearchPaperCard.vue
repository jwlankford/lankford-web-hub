  ...
  >
    ...
    <div>
      <div class="flex items-center justify-between text-xs mb-3 text-slate-500 dark:text-slate-400 font-mono">
        <span class="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-300 dark:border-emerald-500/30 text-emerald-800 dark:text-emerald-300 font-semibold">
      ...
      <h3 
        @click="emit('select', paper)"
        class="text-lg font-bold text-slate-900 dark:text-slate-100 group-hover:text-emerald-600 dark:group-hover:text-emerald-300 cursor-pointer transition-colors leading-snug mb-2 font-serif"
      >
        {{ paper.title }}
      </h3>

      <!-- Authors -->
      <p class="text-xs font-mono text-emerald-700 dark:text-emerald-400/90 mb-4">
        {{ paper.authors }}
      </p>
    ...
    <div>
      ...
      <div v-if="paper.tags && paper.tags.length" class="flex flex-wrap gap-1.5 mb-4">
        <button
          ...
          class="text-xs px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 text-slate-700 dark:text-slate-300 hover:text-emerald-900 dark:hover:text-emerald-200 border border-slate-200 dark:border-slate-700/60 hover:border-emerald-400 dark:hover:border-emerald-500/30 transition-all"
        >
          #{{ tag.name }}
        </button>
      </div>
      ...
      <div class="pt-3 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
        ...
        <span v-else-if="paper.url" class="font-mono text-emerald-700 dark:text-emerald-400/80 flex items-center space-x-1">
        ...
        <div class="flex items-center space-x-3">
          <a
            ...
            @click.stop
            class="text-xs font-semibold text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/80 hover:bg-emerald-100 dark:hover:bg-emerald-900 border border-emerald-300 dark:border-emerald-500/40 px-2.5 py-1 rounded-md flex items-center space-x-1 transition-colors"
          >
