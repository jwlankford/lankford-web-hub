    ...
    >
      ...
      >
        ...
        <div class="flex items-start justify-between border-b border-slate-200 dark:border-slate-800 pb-4 mb-6 relative z-10">
          <div>
            <div class="flex items-center space-x-2 text-xs font-mono text-emerald-700 dark:text-emerald-400 mb-1">
              <span class="px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-950 border border-emerald-300 dark:border-emerald-500/30">
          ...
          <button 
            @click="emit('close')"
            class="text-slate-400 hover:text-slate-700 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 p-2 rounded-full transition-colors ml-4 flex-shrink-0"
            title="Close modal"
          >
        ...
        <div class="space-y-6 text-sm relative z-10 max-h-[70vh] overflow-y-auto pr-1">
          ...
          <div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700/50 space-y-2">
            <div>
              ...
              <p class="text-emerald-800 dark:text-emerald-300 font-mono font-medium">{{ paper.authors }}</p>
            </div>
            <div v-if="paper.zotero_key" class="pt-2 border-t border-slate-200 dark:border-slate-700/40 flex items-center justify-between">
              ...
              <span class="text-xs font-mono bg-slate-100 dark:bg-slate-900 px-2.5 py-1 rounded text-emerald-800 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/20">
                {{ paper.zotero_key }}
              </span>
            </div>
            <div v-if="paper.url" class="pt-2 border-t border-slate-200 dark:border-slate-700/40 flex items-center justify-between">
              ...
              <a 
                ...
                class="text-xs font-mono text-emerald-700 dark:text-emerald-400 hover:underline flex items-center space-x-1 truncate max-w-sm"
              >
          ...
          <div v-if="paper.abstract">
            <h3 class="text-xs font-mono uppercase tracking-wider text-emerald-700 dark:text-emerald-400 mb-2 font-semibold">
              Abstract & Context
            </h3>
          ...
          <!-- Key Findings -->
          <div v-if="paper.key_findings" class="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-300 dark:border-emerald-500/30 p-4 rounded-xl">
            <h3 class="text-xs font-mono uppercase tracking-wider text-emerald-800 dark:text-emerald-400 mb-2 font-semibold flex items-center space-x-1.5">
          ...
          <div v-if="paper.tags && paper.tags.length">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                class="px-3 py-1 rounded-full text-xs font-mono bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-500/40 hover:bg-emerald-200 dark:hover:bg-emerald-800 transition-colors"
              >
                #{{ tag.name }}
              </button>
            </div>
        ...
        <div class="mt-6 pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-end space-x-3 relative z-10">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            class="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-semibold shadow-md shadow-emerald-600/30 transition-all flex items-center space-x-1.5"
          >
